    ...
    : ControllerBase
{
    ...
    /// <param name="newStatus">The new actuality status. <br/>
    /// <c>true</c> - actual. <br/>
    ...
    /// <response code="500">Internal error.</response>
    [HttpPut("project/entry/{id}/change/actuality")]
    [Authorize(Roles = RolesStorage.ProjectUpdateRole)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> ChangeProjectEntryActuality([Required][FromRoute] int id, [Required][FromQuery] bool newStatus)
    {
        string? username =  User.GetUserName();
        if (username == null)
        {
        ...
        using IDisposable? scope = logger.BeginScope("User {username} try change project entry actuality to {newStatus}.", username, newStatus);
        try
        {
            OperationResult result = await units.ChangeProjectEntryActualityAsync(id,  newStatus, HttpContext.RequestAborted);
            switch (result)
            {
                case OperationResult.Success:
    ...
    /// <response code="500">Internal error.</response>
    [HttpGet("project/entry/{id}/images")]
    [Authorize(Roles = RolesStorage.GetProjectsRole)]
    ...
    /// </summary>
    /// <param name="id">The project entry id.</param>
    /// <param name="images">The list of images id.<br/>
    /// Min length - 1; <br/>
    /// Max length - 100.
    /// </param>
    ...
    /// <response code="401">Wrong JWT.</response>
    /// <response code="500">Internal error.</response>
    [HttpPost("project/images/migrate/to/{id}")]
    [Authorize(Roles = RolesStorage.ProjectUpdateRole)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    ...
    public async Task<IActionResult> CopyImagesToProjectAsync([Required] [FromRoute] int id,
        [Required] [FromBody] [MinLength(1)] [MaxLength(100)]IEnumerable<int> images)
    {
        string? username = User.GetUserName();
        if (username == null)
        {
        ...
        try
        {
            ...
            OperationResult result = await units.CopyImagesToNewProjectEntry(imagesArray.ToArray(), id, HttpContext.RequestAborted);
            switch (result)
            {
                case OperationResult.Success:
    ...
    /// <response code="500">Internal error.</response>
    [HttpPut("project/image/{id}/change/active")]
    [Authorize(Roles = RolesStorage.ProjectUpdateRole)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status404NotFound)]
    [ProducesResponseType<ProblemDetails>(StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> ChangeImageActivityAsync([Required] [FromRoute] int id, [Required][FromQuery] bool newState)
    {
        string? username = User.GetUserName();
        if (username == null)
        {
        ...
        using IDisposable? scope = logger.BeginScope("User {username} try change image activity to project {id}.", username, id);
        try
        {
