    ...
    IOptions<JwtOptions> jwtOpts) : ControllerBase
{
    ...
    /// </summary>
    /// <param name="model">
    ///     The role creation model.<br />
    ///     WARNING <see cref="CreateRoleModel.Roles" /> should be same as service roles.
    /// </param>
    /// <returns></returns>
    ...
    private string GenerateJwt(string username, IEnumerable<string>? roles, string sessionId)
    {
        ...
        DateTime? expires = jwtOpts.Value.TokenTimeToLive != null
            ? DateTime.UtcNow.Add(jwtOpts.Value.TokenTimeToLive.Value)
            : null;
        SymmetricSecurityKey key =
            new(Convert.FromBase64String(System.IO.File.ReadAllText(jwtOpts.Value.SecretFilePath)));
