...
public interface IApiKeyProcessor
{
    ...
    /// <returns>
    ///     <see cref="ApiKeyValidationResult.Valid" /> if validated successfully; otherwise another
    ///     <see cref="ApiKeyValidationResult" />.
    /// </returns>
    public Task<(ApiKeyValidationResult, ApiKeyEntity?)> ValidateAsync(string encryptedKey, VmDatabaseContext db,
        IHashHelper hasher);
}
...
public class ApiKeyProcessor(ICryptHelper crypt) : IApiKeyProcessor
{
        ...
        IHashHelper hasher)
    {
        ...
        return DateTimeOffset.UtcNow <= key.ExpiresUtc
            ? (ApiKeyValidationResult.Valid, key)
            : (ApiKeyValidationResult.Expired, key);
    }
}
